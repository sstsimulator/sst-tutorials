...
params = params.Param(config.ncpu, config.nrows*config.ncols)
...
for row in range(config.nrows):
    for col in range(config.ncols):
        ...
        CoreGroup(row*config.ncols+col, config.ncpu, grid[(row, col)], default_latency)
